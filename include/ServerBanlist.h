...
public:
    ...
    void RemoveFromList(const char* name, const char* machineId)
    {
        if (!IsBanned(name, machineId))
            return;
    ...
    bool IsBanned(const char* name, const char* machineId = nullptr)
    {
        for (const auto& player : m_bannedPlayers)
        {
            if (machineId && !strcmp(player.MachineId.c_str(), machineId))
            {
                return true;
            }
